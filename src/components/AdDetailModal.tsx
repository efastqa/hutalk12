  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Real-Time Customer Status Bar */}
        {listing.status === 'pending' ? (
          <div className="bg-gradient-to-r from-amber-500 via-amber-600 to-amber-500 text-white px-4 py-2.5 flex items-center justify-between text-xs shadow-inner">
            <div className="flex items-center gap-2">
              <Clock className="w-4 h-4 text-amber-200 animate-pulse shrink-0" />
              <div>
                <span className="font-bold">Advertisement Status: Under Safety Review</span>
                <span className="hidden sm:inline text-amber-100 ml-1.5">
                  • Only you & admin can see this ad until verified.
                </span>
              </div>
            </div>
            {canManage && (
              <span className="bg-black/25 text-amber-100 text-[10px] font-black uppercase px-2 py-0.5 rounded-full shrink-0">
                Seller Preview Mode
              </span>
            )}
          </div>
        ) : listing.status === 'rejected' ? (
          <div className="bg-red-600 text-white px-4 py-2.5 flex items-center justify-between text-xs shadow-inner">
            <div className="flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-red-200 shrink-0" />
              <div>
                <span className="font-bold">Advertisement Status: Action Required</span>
                <span className="hidden sm:inline text-red-100 ml-1.5">
                  • {listing.verificationNotes || 'Please update your advertisement to meet safety standards.'}
                </span>
              </div>
            </div>
            {canManage && (
              <button
                ...
                className="bg-white text-red-700 font-bold px-2 py-0.5 rounded text-[11px] hover:bg-red-50 cursor-pointer"
              >
                Edit Details
              </button>
            )}
          ...
          <div className="bg-gradient-to-r from-emerald-600 to-teal-700 text-white px-4 py-2 flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-200 shrink-0" />
              <span>
                <strong>Your Ad is Active & Live</strong> across Sri Lanka • <strong>{listing.views || 0}</strong> buyer views
              </span>
            </div>
            <span className="bg-black/20 text-emerald-100 text-[10px] font-black uppercase px-2 py-0.5 rounded-full shrink-0">
              Live Listing
            </span>
          </div>
        ) : null}
